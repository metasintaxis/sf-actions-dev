...
get_log_level_value() {
	local level_name="$1"
	# Convert to uppercase using tr instead of bash expansion for better compatibility
	level_name=$(echo "$level_name" | tr '[:lower:]' '[:upper:]')
	case "$level_name" in
		"DEBUG")
		...
		if should_use_colors; then
			...
			case "$level_name" in
				...
				"WARN")
					...
					;;
				"ERROR")
					printf "\033[31m%s\033[0m\n" "$log_entry" >&2 # Red, to stderr
					;;
				"CRITICAL")
					printf "\033[31;1m%s\033[0m\n" "$log_entry" >&2 # Bright Red, to stderr
					;;
				"ALERT")
					printf "\033[37;41m%s\033[0m\n" "$log_entry" >&2 # White on Red background, to stderr
					;;
				"EMERGENCY" | "FATAL")
					printf "\033[1;37;41m%s\033[0m\n" "$log_entry" >&2 # Bold White on Red background, to stderr
					;;
				"INIT")
					printf "\033[35m%s\033[0m\n" "$log_entry" # Purple for init
					;;
				"SENSITIVE")
					printf "\033[36m%s\033[0m\n" "$log_entry" # Cyan for sensitive
					;;
				*)
					printf "%s\n" "$log_entry" # Default color for unknown level
					;;
			esac
		else
			...
			if [[ "$level_name" == "ERROR" || "$level_name" == "CRITICAL" || "$level_name" == "ALERT" || "$level_name" == "EMERGENCY" || "$level_name" == "FATAL" ]]; then
				printf "%s\n" "$log_entry" >&2 # Error messages to stderr
			else
				printf "%s\n" "$log_entry"
			fi
		fi
